...
function setPageTitle() {
  const pageLabels = {
    home: "Inicio",
    catalog: "Catálogo",
    product: state.currentProduct?.nombre || "Producto",
  };
  document.title = `${pageLabels[pageType()] || "Tienda"} | ${STORE_CONFIG.storeName}`;
}
...
function productUrl(product, contextCategory = "") {
  const params = new URLSearchParams({ id: product.id });
  if (contextCategory) params.set("cat", contextCategory);
...
function productCard(product, options = {}) {
  ...
  const contextCategory = options.contextCategory || "";

  return `
    <article class="product-card">
      <a href="${productUrl(product, contextCategory)}" class="product-media">
      ...
      <div class="product-card__body">
        ...
        <div class="product-card__actions">
          <a href="${productUrl(product, contextCategory)}" class="btn btn-secondary">Ver producto</a>
          <button class="btn btn-primary" data-add-cart="${product.id}" data-qty="1">Agregar al carrito</button>
        </div>
      </div>
    </article>
  `;
...
function renderCollections() {
  ...
  node.innerHTML =
    state.categories
      .map(
        (category) => `
          <a class="collection-card" href="catalogo.html?categoria=${encodeURIComponent(category.nombre)}">
...
function renderNewCollection() {
  ...
  if (!node) return;
  const products = state.products.filter((product) => hasCategory(product, "nueva coleccion")).slice(0, 3);
  node.innerHTML =
    products
      .map(
        (product, index) => `
          <article class="new-collection-card ${index === 0 ? "is-featured" : ""}">
            ...
            <div class="new-collection-card__content">
              ...
              <div class="new-collection-card__actions">
                ...
                <a href="${productUrl(product, "nueva coleccion")}" class="btn btn-primary">Ver drop</a>
              </div>
            </div>
          </article>
        `
...
function renderSale() {
  ...
  if (!node) return;
  const products = state.products.filter((product) => hasCategory(product, "sale"));
  node.innerHTML = products.map((product) => productCard(product, { contextCategory: "sale" })).join("") || `<p class="empty-state">No hay productos en sale todavía.</p>`;
}

function renderCategoryFilter() {
  const select = document.querySelector("[data-filter-category]");
  if (!select) return;
  select.innerHTML = `<option value="all">Todas las categorías</option>${state.categories.map((category) => `<option value="${category.nombre}">${category.nombre}</option>`).join("")}`;
  const categoryFromUrl = new URLSearchParams(window.location.search).get("categoria");
  if (categoryFromUrl) {
    state.filters.category = categoryFromUrl;
...
function filteredProducts() {
  return state.products.filter((product) => {
    ...
    const priceMatch = finalPrice(product) <= Number(state.filters.price || Infinity);
...
function renderCatalogPreview() {
  ...
  if (!grid || pageType() !== "home") return;
  const products = state.products.slice(0, previewLimit());
  grid.innerHTML = products.map((product) => productCard(product, { contextCategory: product.categorias?.[0] || "" })).join("") || `<p class="empty-state">No hay productos para mostrar.</p>`;
  const count = document.querySelector("[data-results-count]");
  if (count) count.textContent = `${products.length} destacados`;
}

function renderCatalogPage() {
  const grid = document.querySelector("[data-product-grid]");
  if (!grid || pageType() !== "catalog") return;
  const products = filteredProducts();
  grid.innerHTML = products.map((product) => productCard(product, { contextCategory: state.filters.category === "all" ? product.categorias?.[0] || "" : state.filters.category })).join("") || `<p class="empty-state">No encontramos productos con esos filtros.</p>`;
  const count = document.querySelector("[data-results-count]");
...
function buildProductWhatsappLink(product, quantity) {
  return `https://wa.me/${STORE_CONFIG.whatsappNumber}?text=${encodeURIComponent(`Hola, quiero consultar por el producto ${product.nombre} (código: ${product.codigo}) en cantidad ${quantity}.`)}`;
}

function addToCart(productId, quantity = 1) {
  const product = state.products.find((item) => item.id === productId) || state.currentProduct;
  if (!product) return;
...
function renderQuantityControl() {
  ...
  if (valueNode) valueNode.textContent = state.productQuantity;
}

function relatedProductsFor(product, contextCategory) {
  ...
  return {
    ...
    items: state.products.filter((item) => item.id !== product.id && hasCategory(item, chosenCategory)).slice(0, 8),
  };
}

function renderRelatedProducts(product, contextCategory) {
  const node = document.querySelector("[data-related-products]");
  if (!node) return;

  const { category, items } = relatedProductsFor(product, contextCategory);
  const title = document.querySelector("[data-related-title]");
  const kicker = document.querySelector("[data-related-kicker]");
  if (title) title.textContent = category ? `Más de ${category}` : "Más productos";
  if (kicker) kicker.textContent = category ? "También te puede gustar" : "Sigue explorando";

  node.innerHTML = items.map((item) => productCard(item, { contextCategory: category })).join("") || `<p class="empty-state">No hay más productos relacionados todavía.</p>`;
}
...
async function renderProductDetail() {
  ...
  const contextCategory = params.get("cat") || "";
      ...
      <div class="product-detail__content">
        ...
        <div class="quantity-control">
          ...
          <div class="quantity-stepper">
            <button class="icon-button" data-qty-minus aria-label="Restar cantidad">−</button>
            <strong data-quantity-value>1</strong>
            <button class="icon-button" data-qty-plus aria-label="Sumar cantidad">+</button>
          </div>
        </div>
        <div class="detail-actions">
          <button class="btn btn-primary" data-add-cart="${product.id}" data-qty-source="product">Agregar al carrito</button>
          <a class="btn btn-whatsapp" href="${buildProductWhatsappLink(product, state.productQuantity)}" data-product-whatsapp target="_blank" rel="noreferrer">Consultar por WhatsApp</a>
        </div>
  ...
  renderRelatedProducts(product, contextCategory);
}

function updateProductWhatsappLink() {
  const link = document.querySelector("[data-product-whatsapp]");
  if (!link || !state.currentProduct) return;
  link.href = buildProductWhatsappLink(state.currentProduct, state.productQuantity);
}
...
function bindEvents() {
  document.body.addEventListener("click", (event) => {
    ...
    if (dot) {
      ...
      state.heroIndex = index;
      slides.forEach((slide, i) => slide.classList.toggle("is-active", i === index));
      dots.forEach((item, i) => item.classList.toggle("is-active", i === index));
    }

    if (thumb && state.currentProduct) {
      const image = state.currentProduct.imagenes?.[Number(thumb.dataset.thumb)];
      const main = document.querySelector("[data-main-product-image]");
      if (main && image) main.src = image;
      document.querySelectorAll(".thumb").forEach((button) => button.classList.toggle("is-active", button === thumb));
    }

    if (event.target.closest("[data-qty-minus]")) {
      state.productQuantity = Math.max(1, state.productQuantity - 1);
      renderQuantityControl();
    ...
    if (addBtn) {
      const qty = addBtn.dataset.qtySource === "product" ? state.productQuantity : Number(addBtn.dataset.qty || 1);
      addToCart(addBtn.dataset.addCart, qty);
    }
  ...
  document.addEventListener("input", (event) => {
    ...
    if (event.target.matches("[data-filter-price-number]")) {
      const value = Number(event.target.value || 0);
      state.filters.price = value > 0 ? value : Infinity;
      renderCatalogPage();
    }
  });

  window.addEventListener("resize", () => {
    if (pageType() === "home") renderCatalogPreview();
  });
}
...
async function init() {
  ...
  updateCartBadges();
  buildHero();
  if (document.querySelector("[data-hero]")) initHeroSlider();
  syncFiltersFromUrl();
  bindEvents();
  ...
  try {
    ...
    console.error(error);
    document.querySelectorAll("[data-sale-products], [data-product-grid], [data-collections], [data-new-collection], [data-product-detail], [data-related-products]").forEach((node) => {
